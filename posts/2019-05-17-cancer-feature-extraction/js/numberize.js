...
$(document).ready(function () {
  ...
  $('h2').each(function (h2_index) {
    ...
    // Go through all table tags between two h2 tags.
    $(this).nextUntil('h2', 'd-table-caption').each(function (table_index) {
      var label = (h2_index + 1) + '.' + (table_index + 1) + '.  ';
      var text = $(this).html();
      // Add index to figcaption elements.
      $(this).prepend('Table ' + label);
      console.log($(this).attr('id'));
      $(this).attr('label', label.trim().slice(0, -1));
      $(this).attr('text', text);
    });
  ...
  // Resolve all table citations
  // -----------------------------------------------------------------------------
  var figurelinks = $("a[href^='#tb-']");
  figurelinks.each(function (index) {
    var id = $(this).attr('href');
    var table = $(id);
    var table_id = table.attr('label');
